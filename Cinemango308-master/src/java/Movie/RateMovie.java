...
public class RateMovie extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int userid=((UserInfoBean)request.getSession().getAttribute("userinfobean")).getUserId();
            int movieid=parseInt(request.getParameter("movieid"));
            double rating=parseDouble(request.getParameter("rating"));
            manager.rateMovie(userid, movieid, rating);
            int size= manager.getNumRatings(movieid);
            RatingBean bean=  (RatingBean) request.getSession().getAttribute("ratingbean");
            bean.setMovieID(movieid);
            bean.setRating(rating);
            bean.setUserID(userid);
            bean.setSize(size);
            request.getSession().setAttribute("ratingbean", bean);
        }
    }
